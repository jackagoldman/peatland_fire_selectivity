...
# Read in rasters
peatland_data <- rast("data/peatland_data.tif")  # Adjust path as needed
canopy_data <- rast("data/canopy_data.tif")     # Adjust path as needed

# Read in shapefile
fire_prog <- st_read("data/fire_prog.shp")        # Adjust path as needed
...
dnbr_files <- list.files("dnbr_raster/", pattern = "*_dnbr.tif$", full.names = TRUE)
...
process_fire <- function(i, prog_poly, canopy_data_classified, peatland_data) {
  ...
  dnbr_raster <- rast(paste0("dnbr_raster/", k_fireid, "_dnbr.tif"))
...
results <- mclapply(1:nrow(prog_poly), process_fire, prog_poly = prog_poly, 
                    canopy_data_classified = canopy_data_classified, 
                    peatland_data = peatland_data, mc.cores = num_cores)
